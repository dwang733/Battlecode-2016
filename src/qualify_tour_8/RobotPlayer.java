...
public strictfp class RobotPlayer {
    ...
    static Direction gap2 = null; // Gap for gardener when building tanks
        ...
        while (true) {
            ...
            try {
                ...
                if (buildTanks) {
                    ...
                    if (rc.getTeamBullets() > RobotType.TANK.bulletCost * 1.5f) {
                        System.out.println("Try to build tank.");
                        tryBuildUnit(RobotType.TANK);
                    }
                    if (tankCnt < 100) {
                        rc.broadcastInt(LAST_GARDENER_SETTLED_IDX, rc.getRoundNum());
                        tankCnt++;
                    ...
                    while (!settled) {
//                    MapLocation closestGardenerLoc = findNewGardenerSpot();
//                    if (closestGardenerLoc != null && closestGardenerLoc != targetLoc) {
//                        System.out.println("Found new closest gardener spot.");
//                        mLineStart = myLoc;
//                        prevDir = null;
//                        foundSpotFromBroadcast = true;
//                    }
//                    else if (closestGardenerLoc == null) {
//                        System.out.println("No closest gardener spot.");
//                        mLineStart = null;
//                        prevDir = null;
//                        foundSpotFromBroadcast = false;
//                    }
                        ...
                        if (targetLoc.distanceTo(myLoc) < 0.1f) {
                    ...
                    while (buildDisruptiveSoldier) {
                        ...
                        // If enemy archon's close, don't build the disruptive scout
...
//                            break;
    ...
    static MapLocation findNewGardenerSpot() throws GameActionException {
        ...
        return null;
//        if (freeGardenerLocs.size() == 0) return null;
//        float closestDist = Integer.MAX_VALUE;
//        MapLocation closestLoc = null;
//        int cnt = 0;
//        for (MapLocation freeGardenerLoc : freeGardenerLocs) {
//            if (freeGardenerLoc.x == -1 || freeGardenerLoc.y == -1) {
////                freeGardenerLocs.remove(freeGardenerLoc);
//                continue;
//            }
//            if (myLoc.distanceTo(freeGardenerLoc) < closestDist) {
//                closestDist = myLoc.distanceTo(freeGardenerLoc);
//                closestLoc = freeGardenerLoc;
//                gardenerIdxCnt = cnt;
//            }
//            cnt++;
//        }
//        return closestLoc;
     ...
     * @throws GameActionException
     */
    static void tankRush() throws GameActionException {
        System.out.println("Tank rush.");

        // Plant a tree if possible (always leave a gap for building units)
        for (int i=0;i<6;i++) {
            Direction dir = new Direction((float) (i * Math.PI / 3));
            if (gap == null && rc.canPlantTree(dir)) {
                System.out.println("Set gap.");
                gap = dir;
            }
            else if (gap2 == null && rc.canPlantTree(dir)) {
                System.out.println("Set gap2.");
                gap2 = dir;
            }
            // Check that direction is not the gap
            else if (!gap.equals(dir, (float) 1e-3) && !gap2.equals(dir, (float) 1e-3) && rc.canPlantTree(dir)) {
                System.out.println("Planting tree.");
                rc.plantTree(dir);
                break;
            }
        ...
        // If there's nearby bullets or archons, build more tanks
        if (rc.senseNearbyBullets(-1).length > 0 || leaderArchonStatus == ArchonStatus.CRAMPED ||
                leaderArchonStatus == ArchonStatus.CLOSE) {
            System.out.println("Increased tank output");
            TANK_COOLDOWN = 35;
            TANK_THRESHOLD = RobotType.SOLDIER.bulletCost * 1.3f;
        }
        else {
            System.out.println("Regular tank output");
            TANK_COOLDOWN = 30;
            TANK_THRESHOLD = RobotType.SOLDIER.bulletCost * 2.0f;
        }

        System.out.println(turnsSinceTank + " " + TANK_COOLDOWN);
        System.out.println(rc.getTeamBullets() + " " + TANK_THRESHOLD);
        if (turnsSinceTank > TANK_COOLDOWN && rc.getTeamBullets() > TANK_THRESHOLD) {
