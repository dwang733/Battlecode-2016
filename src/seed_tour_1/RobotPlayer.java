...
public strictfp class RobotPlayer {
    ...
    static int GARDENER_COOLDOWN = 20;
    static float GARDENER_THRESHOLD = RobotType.GARDENER.bulletCost * 1.5f;
    static int LAST_GARDENER_SETTLED_IDX; // Don't build new gardener until old one settled
    static int turnsSinceGardener = 0;
    ...
    static int turnsSinceSoldier = 0;
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        if (rc.getRoundNum() < 5) {
            rc.broadcastBoolean(LAST_GARDENER_SETTLED_IDX, true);
        }
    ...
    static void archonBuild() throws GameActionException {
        boolean lastGardenerSettled = rc.readBroadcastBoolean(LAST_GARDENER_SETTLED_IDX);
        if (turnsSinceGardener < GARDENER_COOLDOWN && rc.getTeamBullets() > GARDENER_THRESHOLD && lastGardenerSettled) {
            if (tryBuildUnit(RobotType.GARDENER) != null) {
                turnsSinceGardener = 0;
                rc.broadcastBoolean(LAST_GARDENER_SETTLED_IDX, false);
            }
        }
        turnsSinceSoldier++;
    }
        ...
        while (true) {
            ...
            try {
                ...
                while (!settled) {
                    ...
                    // FIXME: Keep in mind that gardener might not be able to reach targetLoc (b/c trees, etc)
                    else {
                        System.out.println("Moving towards viable gardener loc.");
                        prevPrevLoc = prevLoc;
                        prevLoc = myLoc;
                        tryMove(targetLoc, 10, 17); // FIXME: Tweak values
        ...
        while (true) {
            ...
            try {
                ...
                if (targetLoc != null) {
                    ...
                    if (rc.getLocation().distanceTo(targetLoc) < 3.0f) {
                        targetLoc = null;
                    }
                    else {
                        prevPrevLoc = prevLoc;
