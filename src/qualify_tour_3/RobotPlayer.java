...
//                        tryMoveAdvanced(targetLoc, 10, 35);
...
//                    System.out.println(nearbyGardener.location.distanceTo(desiredLoc) + " " + bufferDist);
    ...
    static boolean bugPathing(MapLocation loc) throws GameActionException {
        ...
        if (rc.canMove(loc)) {
            rc.move(loc);
            prevVisitedLocs.add(loc);
            return true;
        }
        // Check if can move directly in direction of location
        Direction desiredDir = myLoc.directionTo(loc);
        rc.setIndicatorLine(myLoc, myLoc.add(desiredDir, 2), 0, 255, 0);
        if (rc.canMove(desiredDir)) {
            rc.move(desiredDir);
            prevVisitedLocs.add(myLoc.add(desiredDir, rc.getType().strideRadius));
            return true;
        }
        // Else follow the obstacle
        else {
            // First time encountering obstacle, find the direction to move in (default turning left)
            for (int i = 1; i < 36; i++) {
                Direction testDir = desiredDir.rotateLeftDegrees(i * 10);
                if (rc.canMove(testDir) && !prevVisitedLocs.contains(myLoc.add(testDir, rc.getType().strideRadius))) {
                    rc.setIndicatorLine(myLoc, myLoc.add(testDir, 2), 0, 0, 0);
                    rc.move(testDir);
                    prevDir = testDir;
                    prevVisitedLocs.add(myLoc.add(testDir, rc.getType().strideRadius));
                    return true;
                }
            }
        }
