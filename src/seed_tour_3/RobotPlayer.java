...
public strictfp class RobotPlayer {
    ...
    static int GARDENER_COOLDOWN = 50;
                ...
                while (!settled) {
                    ...
                    if (targetLoc.distanceTo(myLoc) < 0.01f) {
                        ...
                        if (rc.canSenseAllOfCircle(targetLoc, RobotType.GARDENER.bodyRadius) &&
                            ...
                            System.out.println("Current spot invalid, pick a new one.");
                            targetLoc = findNewGardenerSpot();
                            if (targetLoc == null) {
                                targetLoc = myLoc;
                            }
...
//            }
    ...
    static boolean tryMoveAdvanced(MapLocation loc, float degreeOffset, int checksPerSide) throws GameActionException {
        ...
        // First, try intended direction
        if (rc.canMove(loc)) {
            System.out.println("Nvm, could move in that direction");
            rc.move(loc);
            return true;
        }
        ...
        while(currentCheck<=checksPerSide) {
            ...
            if(rc.canMove(desiredLoc)) {
                ...
                for (RobotInfo nearbyGardener : nearbyGardeners) {
                    ...
                    System.out.println(MapLocation.doCirclesCollide(nearbyGardener.location, bufferDist, desiredLoc,
